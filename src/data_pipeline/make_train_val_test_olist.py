"""
SIDN-CRO data pipeline -- Cross-replication (Stage 4 / Olist).

Builds features, augmented target and Train / Validation / Test split for
the Olist multi-product weekly panel using the same augmentation parameters
as the DataCo branch (k1=k2=0.030, k3=0.040, alpha_inc=0.80, eps_sd=0.05).

Splitting (chronological)
    2016-09-05  ...  2018-01-31    Train       (~70 %)
    2018-02-01  ...  2018-05-31    Validation  (~15 %)
    2018-06-01  ...  2018-08-29    Test        (~15 %)

The Olist panel has weak activity (<70 %) before October 2016, so we trim
to 2016-10-01 onwards.  The 2018-Q2/Q3 test window naturally exhibits
covariate shift relative to the training period.
"""
...
from src.utils.paths import (OLIST_INTERMEDIATE_DIR, OLIST_TRAIN_DIR,
                             OLIST_VAL_DIR, OLIST_TEST_DIR,
                             OLIST_PREPROCESSED_DIR)

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("make_split_olist")
...
PANEL_START = pd.Timestamp("2016-10-03")
PANEL_END   = pd.Timestamp("2018-08-29")
TRAIN_END   = pd.Timestamp("2018-02-01")
VAL_END     = pd.Timestamp("2018-06-01")

CONT_FEATS = ["p", "freight",
              "b_s", "b_m", "b_r",
              "u_s", "u_m", "u_r",
              "weight",
...
def trim_to_active_window(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    log.info("Trimmed inactive tail: %d -> %d (-%d)",
...
def add_engineered(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    panel["year_norm"] = (panel["year"] - 2016) / 2.0
...
def main() -> None:
    ...
    panel = pd.read_parquet(OLIST_INTERMEDIATE_DIR / "olist_weekly_full.parquet")
    ...
    joblib.dump(x_scaler, OLIST_PREPROCESSED_DIR / "x_scaler.joblib")
    joblib.dump(y_scaler, OLIST_PREPROCESSED_DIR / "y_scaler.joblib")

    log.info("Writing splits:")
    write_split("train", train, OLIST_TRAIN_DIR, x_scaler, y_scaler)
    write_split("val",   val,   OLIST_VAL_DIR,   x_scaler, y_scaler)
    write_split("test",  test,  OLIST_TEST_DIR,  x_scaler, y_scaler)
    ...
    schema = {
        ...
        "augmentation_params": {
            "k1": K1, "k2": K2, "k3": K3,
            "alpha_inc": ALPHA_INC, "noise_sd": NOISE_SD, "seed": SEED,
        },
        "split": {
    ...
    with open(OLIST_PREPROCESSED_DIR / "schema.json", "w") as f:
